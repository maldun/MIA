...
def start_ollama(first_setup,version="lama3.2",docker="docker",timeout=10,mode="rocm",port=11434,name="ollama",**_):
    if first_setup is True: # download model
      ...
      # only have to start serve
      cmd = [docker] + f"exec -it {name} ollama serve ".split()
      proc = subprocess.Popen(cmd)
      proc.wait()
